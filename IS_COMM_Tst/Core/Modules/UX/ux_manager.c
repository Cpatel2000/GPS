...
DWchar gpsNSIndic = {"%c", ".", 0, 0, true, '-'};
DWchar gpsEWIndic = {"%c", ".", 0, 0, true, '-'};

DWfloat gpsLat = {"%5.2f", "---", 0, 0, true, 0};
DWfloat gpsLon = {"%5.2f", "---", 0, 0, true, 0};
...
#pragma diag_suppress= Pa149
  // current screen is out
  switch (lastScreen) {
  }
  
  
  //  before screen is switched in
  switch (screen_no) {
  }
#pragma diag_warning= Pa149
  ...
  case GPS_OLED:
    SSD1306_Clear();
    SSD1306_GotoXY (0,0);
    SSD1306_Puts ("GPS", &Font_11x18, SSD1306_COLOR_WHITE);
    SSD1306_GotoXY (0, 20);
    SSD1306_Puts ("Lat: ", &Font_11x18, SSD1306_COLOR_WHITE);
    gpsLat.xPos = 65;
    gpsLat.yPos = 20;
    SSD1306_GotoXY (0, 41);
    SSD1306_Puts ("Lon:", &Font_11x18, SSD1306_COLOR_WHITE);
    gpsLon.xPos = 65;
    gpsLon.yPos = 41;
    break;
  }
...
void UpdateScreenValues(void)
{
  ...
  case SHOW_COUNT_N_BUTTONS:
    ...
    break;
  case GPS_OLED:
    SSD1306_GotoXY(gpsLat.xPos, gpsLat.yPos);
    if (gpsLat.valid) {
      sprintf(displayString, gpsLat.format, gpsLat.data);
      SSD1306_Puts(displayString, &Font_11x18, SSD1306_COLOR_WHITE);
    }
    else
      SSD1306_Puts(gpsLat.invalidMsg, &Font_11x18, SSD1306_COLOR_WHITE);
    
    SSD1306_GotoXY(gpsLon.xPos, gpsLon.yPos);
    if (gpsLon.valid) {
      sprintf(displayString, gpsLon.format, gpsLon.data);
      SSD1306_Puts(displayString, &Font_11x18, SSD1306_COLOR_WHITE);
    }
    else
      SSD1306_Puts(gpsLon.invalidMsg, &Font_11x18, SSD1306_COLOR_WHITE);
